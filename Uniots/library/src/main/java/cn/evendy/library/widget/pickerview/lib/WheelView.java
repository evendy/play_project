package cn.evendy.library.widget.pickerview.lib;
...
import cn.evendy.library.widget.pickerview.adapter.WheelAdapter;
import cn.evendy.library.widget.pickerview.listener.OnItemSelectedListener;
...
public class WheelView extends View {
    public final static int IndicatorType_Divider = 0;
    public final static int IndicatorType_Triangle_LEFT = 1;
    public final static int IndicatorType_Triangle_RIGHT = 2;
    public final static int IndicatorType_NULL = 4;

    private int indicatorType;//指示器的类型(横线,小三角)
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        dividerColor = getResources().getColor(R.color.pickerview_wheelview_indicator_color);
        labelTextColor = getResources().getColor(R.color.pickerview_wheelview_labelcolor);
        ...
        if (attrs != null) {
            ...
            labelTextColor = a.getColor(R.styleable.wheelview_labelTextColor, labelTextColor);
            itemsVisible = a.getInt(R.styleable.wheelview_items_visible, itemsVisible);
        }
    ...
    public final void setTextSize(float textSize) {
        if (textSize > 0.0F && !customTextSize) {
            textSize = (int) (context.getResources().getDisplayMetrics().density * textSize);
            paintOuterText.setTextSize(textSize);
            paintCenterText.setTextSize(textSize);
        }
    }
    ...
    public void setLabelTextSize(float labelTextSize) {
        if (labelTextSize > 0.0F && labelTextSize <= textSize && !customTextSize) {
            labelTextSize = (int) (context.getResources().getDisplayMetrics().density * labelTextSize);
            paintLabelText.setTextSize(labelTextSize);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (label != null) {
            ...
            float drawLabelContentStart = getLabelStart();
            float drawLabelContentTop = getLabelTop();
            canvas.drawText(label, drawLabelContentStart, drawLabelContentTop, paintLabelText);
    ...
    private void drawPickerIndicator(Canvas canvas) {
        switch (indicatorType) {
            case IndicatorType_Divider:
                //中间两条横线
                canvas.drawLine(0.0F, firstLineY, measuredWidth, firstLineY, paintIndicator);
                canvas.drawLine(0.0F, secondLineY, measuredWidth, secondLineY, paintIndicator);
                break;
            case IndicatorType_Triangle_LEFT:
                  /*画一个实心三角形*/
                Rect rect = new Rect();
                paintCenterText.getTextBounds("9", 0, 1, rect);
                Path path = new Path();
                path.moveTo(rect.width(), centerY - rect.height() * 4 / 5);
                path.lineTo(rect.width(), centerY - rect.height() * 1 / 5);
                path.lineTo(rect.width() * 1.5f, centerY - rect.height() / 2);
                path.close();
                canvas.drawPath(path, paintIndicator);
                break;
            case IndicatorType_Triangle_RIGHT:
                  /*画一个实心三角形*/
                Rect rect2 = new Rect();
                paintCenterText.getTextBounds("9", 0, 1, rect2);
                Path path2 = new Path();
                path2.moveTo(measuredWidth - rect2.width(), centerY - rect2.height() * 4 / 5);
                path2.lineTo(measuredWidth - rect2.width(), centerY - rect2.height() * 1 / 5);
                path2.lineTo(measuredWidth - rect2.width() * 1.5f, centerY - rect2.height() / 2);
                path2.close();
                canvas.drawPath(path2, paintIndicator);
                break;
        }
    ...
    private float getLabelStart() {
        Rect rect = new Rect();
        paintCenterText.getTextBounds("99", 0, 2, rect);
        return measuredWidth / 2 + rect.width() / 2 + 16;
    }

    private float getLabelTop() {
        Rect rect = new Rect();
        paintCenterText.getTextBounds("99", 0, 2, rect);
        return centerY - rect.height() / 2 - 6;
    }
    ...
    public void setIndicatorType(int indicatorType) {
        ...
        if (indicatorType == IndicatorType_Divider)
            paintIndicator.setColor(dividerColor);
        else
            paintIndicator.setColor(textColorCenter);
    }
